...
public class RedisClient {
    ...
    private static final int blockingQueueNum = System.getProperty("request.blockingqueue.number") == null ? 10 : Integer.valueOf(System.getProperty("request.blockingqueue.number").toString());
    private static final int queueDataNum = System.getProperty("request.everyqueue.data.length") == null ? 100 : Integer.valueOf(System.getProperty("request.everyqueue.data.length").toString());
    ...
    private void KeyOperate() {
        ...
        // 判断key否存在
        logger.info("判断key999键是否存在：" + shardedJedis.exists("key999"));
        logger.info("新增key001,value001键值对：" + shardedJedis.set("key001", "value001"));
        logger.info("判断key001是否存在：" + shardedJedis.exists("key001"));
        // 输出系统中所有的key
        logger.info("新增key002,value002键值对：" + shardedJedis.set("key002", "value002"));
        ...
        logger.info("系统中删除key002: " + jedis.del("key002"));
        logger.info("判断key002是否存在：" + shardedJedis.exists("key002"));
        // 设置 key001的过期时间
        logger.info("设置 key001的过期时间为5秒:" + jedis.expire("key001", 5));
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
        }
        // 查看某个key的剩余生存时间,单位【秒】.永久生存或者不存在的都返回-1
        logger.info("查看key001的剩余生存时间：" + jedis.ttl("key001"));
        // 移除某个key的生存时间
        logger.info("移除key001的生存时间：" + jedis.persist("key001"));
        logger.info("查看key001的剩余生存时间：" + jedis.ttl("key001"));
        // 查看key所储存的值的类型
        logger.info("查看key所储存的值的类型：" + jedis.type("key001"));
        jedis.flushDB();
        /*
    ...
    private void StringOperate() {
         ...
         */
        logger.info("一次性新增key201,key202,key203,key204及其对应值：" + jedis.mset("key201", "value201",
                "key202", "value202", "key203", "value203", "key204", "value204"));
        logger.info("一次性获取key201,key202,key203,key204各自对应的值：" +
    ...
    private void SetOperate() {
        ...
        logger.info("查看sets集合中的所有元素:" + jedis.smembers("sets"));
        /*logger.info("sets集合中任意位置的元素出栈："+jedis.spop("sets"));//注：出栈元素位置居然不定？--无实际意义
        logger.info("查看sets集合中的所有元素:"+jedis.smembers("sets"));*/
